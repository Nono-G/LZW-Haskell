...
lzwEncode0 table [] = []
lzwEncode0 table texte = let (pref, mc, suff) = split table texte
			in if suff == []
				then [(fromJust mc)]
				else ((fromJust mc):(lzwEncode0 (insert table (pref++[(head suff)])) suff))

--ENCODE AVEC ACCUMULATEUR, AVEC DEBUG : AFFICHAGE DE LA TABLE AVEC
lzwEncode0D :: Table a => a -> String -> ([Code],a)
lzwEncode0D table texte = let (pref, mc, suff) = split table texte
			in if suff == []
				then ([(fromJust mc)],table)
				else let (cs, t) = (lzwEncode0D (insert table (pref++[(head suff)])) suff)
					in (((fromJust mc):cs),t)	
